...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	while($i<$n){
		if($stmt = $mysqli->prepare("INSERT INTO upvotes (feed_id, user_id, value, type) VALUES (?,?,?,?)")){
			$stmt->bind_param("ssss", $upvotelist[$i]['feed_id'], $upvotelist[$i]['user_id'], $upvotelist[$i]['value'], $upvotelist[$i]['type']);
			$stmt->execute();
			$stmt->close();
			$i = $i + 1;
		}
