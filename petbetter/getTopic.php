...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($stmt = $mysqli->prepare("SELECT t._id AS _id, t.creator_id AS creator_id, t.topic_name AS topic_name, 
	...
	ON t.creator_id = u._id WHERE t._id= ? AND t.is_deleted != 1")){
		$stmt->bind_param("s", $_id);
		$stmt->execute();
		$stmt->bind_result($_id, $creator_id, $topic_name, $topic_desc, $date_created, $is_deleted, $first_name, $last_name);
		$stmt->store_result();
