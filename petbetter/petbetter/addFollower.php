...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if(!(isset($_POST['_id']) and isset($_POST['topic_id']) and isset($_POST['user_id']))){
		echo 'Details are lacking';
		exit(0);
	}

	if($stmt = $mysqli->prepare("INSERT INTO followers (_id, topic_id, user_id) VALUES (?,?,?)")){
		$stmt->bind_param("sss", $_id, $topic_id, $user_id);
		$stmt->execute();
		$stmt->close();
		echo 'Follower added';
