...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($stmt = $mysqli->prepare("SELECT p._id AS _id, p.user_id AS user_id, p.topic_name AS topic_name, p.topic_content AS topic_content, p.topic_id AS topic_id, 
		p.date_created AS date_created, p.post_photo AS post_photo, p.id_link AS id_link, p.is_deleted AS is_deleted FROM bookmarks AS b INNER JOIN posts AS p ON b.item_id = p._id 
		WHERE b.bookmark_type = 2 AND b.user_id = ? AND p.is_deleted = 0 ORDER BY p.date_created")){
