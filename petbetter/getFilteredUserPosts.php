...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($order==1){
		if($stmt = $mysqli->prepare("SELECT * FROM posts WHERE is_deleted = 0 AND user_id = ? ORDER BY date_created DESC")){
	...
	if($order==2){
		if($stmt = $mysqli->prepare("SELECT p._id AS _id, p.user_id AS user_id, p.topic_name AS topic_name, p.topic_content AS topic_content, p.topic_id AS topic_id, p.date_created AS date_created, 
			...
			p._id = u.feed_id WHERE p.is_deleted = 0 AND p.user_id = ? GROUP BY p._id ORDER BY SUM(u.value) DESC")){
			$stmt->bind_param("s", $user_id);
			$stmt->execute();
